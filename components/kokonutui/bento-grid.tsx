...
const bentoItems: BentoItem[] = [
  {
    ...
    feature: "spotlight",
    spotlightItems: [
      "RSSM latent dynamics + KL balancing",
      "Actor-critic on imagined rollouts",
      "Horizon ablation H = 5 / 15 / 30",
      "Categorical vs Gaussian latents",
      "Crafter score @ 1M steps",
    ],
    size: "lg",
  },
  {
    id: "grpo",
    title: "Reasoning via GRPO / RLVR",
    description:
      "Post-train a small LM with verifiable rewards on math and measure the test-time-compute scaling curve.",
    href: "#",
    feature: "typing",
    typingText:
      "def grpo_step(prompts, policy, verifier):\n  groups = [policy.sample(p, k=8) for p in prompts]\n  rewards = [[verifier(y) for y in g] for g in groups]\n  adv = normalize_within_group(rewards)\n  # clipped policy-gradient, no value net\n  return pg_loss(policy, groups, adv)",
    size: "md",
  },
  {
    id: "dlm",
    ...
    feature: "timeline",
    timeline: [
      { year: "M1", event: "AR twin baseline on FineWeb-Edu" },
      { year: "M2", event: "Masked-diffusion objective (LLaDA-style)" },
      { year: "M3", event: "Matched-FLOPs eval: ppl + gen quality" },
      { year: "M4", event: "Write-up: when does diffusion win?" },
    ],
    size: "md",
  },
  {
